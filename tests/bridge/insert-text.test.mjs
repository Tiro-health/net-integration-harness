/*
 * ui.form.insertText — host-driven typing into the focused field.
 *
...
test("text is inserted at the caret of the focused field", async () => {
    ...
    const result = deliver(h.window, "ui.form.insertText", { text: "no acute distress" });
...
test("insertion falls back to the last-focused field once focus has left the page", async () => {
    ...
    const result = deliver(h.window, "ui.form.insertText", { text: "afebrile" });
...
test("a field inside the form-filler's shadow root is found", async () => {
    ...
    const result = deliver(h.window, "ui.form.insertText", { text: "10 mm" });
...
test("a contenteditable answer is a valid target", async () => {
    ...
    assert.equal(deliver(h.window, "ui.form.insertText", { text: "dictated" }).inserted, true);
    assert.equal(h.execCommands.length, 1);
});
...
test("with nothing focused the insert is refused, not guessed at", async () => {
    ...
    const result = deliver(h.window, "ui.form.insertText", { text: "conclusion" });
...
test("fields that don't take typed text are not targets", async () => {
    ...
    for (const el of [
        ...
        assert.equal(
            deliver(h.window, "ui.form.insertText", { text: "x" }).inserted, false,
...
test("a textarea is a target", async () => {
    ...
    assert.equal(deliver(h.window, "ui.form.insertText", { text: "x" }).inserted, true);
});
...
test("an empty payload is refused without touching the field", async () => {
    ...
    assert.equal(deliver(h.window, "ui.form.insertText", {}).inserted, false);
    assert.equal(deliver(h.window, "ui.form.insertText", { text: "" }).inserted, false);
    assert.equal(deliver(h.window, "ui.form.insertText", undefined).inserted, false);
    assert.deepEqual(h.execCommands, []);
});
...
test("when execCommand refuses, the value is spliced through the prototype setter", async () => {
    ...
    const result = deliver(h.window, "ui.form.insertText", { text: "at rest " });
...
test("the outcome rides back to the host on the ack", async () => {
    ...
    h.receive({ messageId: "m1", messageType: "ui.form.insertText", payload: { text: "hi" } });
    await flush();
...
test("the page is told what happened", async () => {
    ...
    deliver(h.window, "ui.form.insertText", { text: "note" });

    const fired = h.fired("tiro-text-inserted");
