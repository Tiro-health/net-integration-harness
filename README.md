...
- **`User-Agent`**: every request carries `Tiro.Health.FormSdk.Client/<version>`, which the SDC server aggregates to learn which harness versions are deployed (informs the support window; [atticus-backend#3568](https://github.com/Tiro-health/atticus-backend/issues/3568)). It is set **per request**, so an injected/shared `HttpClient` is never mutated, and a `User-Agent` you configured on that client is preserved ahead of ours
